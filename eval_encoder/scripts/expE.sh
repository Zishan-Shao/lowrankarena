...
# ────────────────────────────────────────────────────────────────────────────
#   cd lowrankarena/
#   bash eval_encoder/scripts/expE.sh                        # all four phases
#   PHASES="e2"   bash eval_encoder/scripts/expE.sh           # E-2 only
#   PHASES="e3a"  TASKS="mnli" bash eval_encoder/scripts/expE.sh
#   PHASES="e3b"  TASKS="mnli" METHODS="svd" bash eval_encoder/scripts/expE.sh
#
...
WARMUP="${WARMUP:-50}"
MEASURE="${MEASURE:-100}"
OUT_E3A="${OUT_E3A:-eval_encoder/eval_results/expE_train_timing.csv}"

# E-3b
E3B_EVAL_STEPS="${E3B_EVAL_STEPS:-0 200 500 1000}"
E3B_NUM_EPOCHS="${E3B_NUM_EPOCHS:-3}"
E3B_EVAL_BACKENDS="${E3B_EVAL_BACKENDS:-naive}"
OUT_E3B="${OUT_E3B:-eval_encoder/eval_results/expE_recovery.csv}"
...
if [[ "${PHASES}" == *"e2"* ]]; then
    ...
    OUT_CSV="${OUT_EXPB}" \
    bash eval_encoder/scripts/expB.sh || rc=$?
...
if [[ "${PHASES}" == *"e3b"* ]]; then
    ...
    for TASK in ${TASKS}; do
        for METHOD in ${METHODS}; do
            ...
            echo "  ── ${TASK} / ${METHOD}  (${MODEL_DIR})"
            rc=0
            python eval_encoder/scripts/run_recovery_curve.py \
                --model_dir     "${MODEL_DIR}" \
                --task          "${TASK}" \
