...
def _get_rope_theta(config: Any, default: float = 10000.0) -> float:
    ...
    if rope_params is not None:
        ...
        try:
            ...
            pass
        try:
            val = getattr(rope_params, "theta", None)
            if val is not None:
                return float(val)
        except Exception:
            pass
...
class SVD_LlamaAttention(_SVD_LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Qwen3 can use sliding-window attention at long context. HF may provide only a 2D padding mask, while the
        # original attention expects a sliding-window causal pattern. Since our SVD attention uses SDPA, we rebuild
        # an SDPA-compatible 4D boolean mask when needed.
        ...
        if sliding_window is not None and (not output_attentions):
            try:
                ...
                try:
                    # Newer Transformers may accept `position_ids` and return a packed-sequence mask.
                ...
                if not isinstance(_out, tuple):
                    raise TypeError(f"Unexpected _preprocess_mask_arguments return type: {type(_out)}")
                if len(_out) == 5:
                    early_exit, am_prep, packed_seq_mask, kv_len, kv_off = _out
                elif len(_out) == 4:
                    early_exit, am_prep, kv_len, kv_off = _out
                else:
                    raise ValueError(f"Unexpected _preprocess_mask_arguments tuple length: {len(_out)}")
                ...
                if early_exit and am_prep is not None:
                    # Respect pre-built tensor masks (e.g., HF SDPA 4D masks). Avoid passing BlockMask to SDPA.
                    if torch.is_tensor(am_prep):
                        attention_mask = am_prep
                else:
                    # Build an SDPA-compatible 4D boolean mask for sliding-window attention.
