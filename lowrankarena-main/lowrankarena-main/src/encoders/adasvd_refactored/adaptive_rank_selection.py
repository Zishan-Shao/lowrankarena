...
class MaskedSVDLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Ensure dtype consistency with input
...
def parameter_budget(op_list: List[MaskedSVDLinear], masks: List[torch.Tensor], p: float):
    ...
    for op, m in zip(op_list, masks):
        ...
        # FIX: Budget relative to ORIGINAL model params, not SVD params
        T_original = T_original + (op.in_features * op.out_features)
    Tmax = p * T_original  # Target is p fraction of original model
    # FIX: Two-sided budget constraint - penalize both over and under budget
    # Use squared relative error: ((Tm - Tmax) / Tmax)^2
    return ((Tm - Tmax) / (Tmax + 1e-12)) ** 2
