...
class SVD_LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_states = self.k_u_proj(self.k_v_proj(hidden_states)).view(bsz, q_len, kv_heads, self.head_dim).transpose(1, 2)
        value_states = self.v_u_proj(self.v_v_proj(hidden_states)).view(bsz, q_len, kv_heads, self.head_dim).transpose(1, 2)
        ...
        if compat_attention:
            ...
            use_am = False
            if attn_mask_sdpa is not None and attn_mask_sdpa.dim() == 4:
                # Expected mask shape: [B, 1 or H, Q, K]
                Bm, Hm, Qm, Km = attn_mask_sdpa.shape
            ...
            attn_output = F.scaled_dot_product_attention(
                q, k, v, attn_mask=attn_mask_sdpa, dropout_p=0.0, is_causal=(attn_mask_sdpa is None)
            )
            ...
            if not torch.isfinite(attn_output).all():
                ...
                try:
                    attn_output = F.scaled_dot_product_attention(
                        q32, k32, v32, attn_mask=am32, dropout_p=0.0, is_causal=(am32 is None)
                    ).to(q.dtype)
                except Exception:
