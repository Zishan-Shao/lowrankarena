...
def flashsvd_rope_sdpa(
    ...
    for nk in range(0, M, BN):
        ...
        for d0 in range(0, dh, BDH):
            ...
            # Older Triton versions don't support tensor slicing like q[:, :half],
            # so we project directly into two half blocks.
            Vq0 = tl.load(
                Vq_ptr + hid*sVq_h + offs_d[:, None]*sVq_dh_in + offs0[None, :]*sVq_dh_out,
                mask=True, other=0.0
            )  # [dh, dh/2]
            Vq1 = tl.load(
                Vq_ptr + hid*sVq_h + offs_d[:, None]*sVq_dh_in + offs1[None, :]*sVq_dh_out,
                mask=True, other=0.0
            )  # [dh, dh/2]
            Vk0 = tl.load(
                Vk_ptr + hid*sVk_h + offs_d[:, None]*sVk_dh_in + offs0[None, :]*sVk_dh_out,
                mask=True, other=0.0
            )  # [dh, dh/2]
            Vk1 = tl.load(
                Vk_ptr + hid*sVk_h + offs_d[:, None]*sVk_dh_in + offs1[None, :]*sVk_dh_out,
                mask=True, other=0.0
            )  # [dh, dh/2]

            # Compute Q/K halves directly: [BM, dh]@[dh, dh/2] and [BN, dh]@[dh, dh/2]
            ...
            if sbq_dh != 0:
                bq0 = tl.load(bq_ptr + hid*sbq_h + offs0*sbq_dh, mask=True, other=0.0)
                bq1 = tl.load(bq_ptr + hid*sbq_h + offs1*sbq_dh, mask=True, other=0.0)
                q0 += bq0[None, :]; q1 += bq1[None, :]
            if sbk_dh != 0:
                bk0 = tl.load(bk_ptr + hid*sbk_h + offs0*sbk_dh, mask=True, other=0.0)
                bk1 = tl.load(bk_ptr + hid*sbk_h + offs1*sbk_dh, mask=True, other=0.0)
