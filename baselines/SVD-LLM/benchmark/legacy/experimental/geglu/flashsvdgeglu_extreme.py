...
def _lowp_kind_from_dtype(dtype: torch.dtype) -> int:
...
def fused_ffn_phase1_geglu(
    ...
    LOWP_KIND: tl.constexpr,  # 0=fp16, 1=bf16, 2=fp32/no-cast
    ...
    for d0 in range(0, D, BD):
        ...
        U2_blk = tl.load(U2_ptr + d[:,None]*sU2_d + offs_r2[None,:]*sU2_r2,
                         mask=m_d[:,None] & (offs_r2[None,:] < R2), other=0.0)
        # TC-friendly matmul path with explicit dtype selection for compiler stability.
        if LOWP_KIND == 1:
            H_low = H.to(tl.bfloat16)
        elif LOWP_KIND == 0:
            H_low = H.to(tl.float16)
        else:
            H_low = H
        acc += tl.dot(H_low, U2_blk)
...
def fused_ffn_phase12_geglu(
    ...
    for d0 in range(0, D, BD):
        ...
        for r2_0 in range(0, R2, BR2):
            ...
            G_acc += tl.dot(U2_blk, V2_blk)  # [BD,BH]

        # TC-friendly matmul path with explicit dtype selection for compiler stability.
        if LOWP_KIND == 1:
            H_low = H_blk.to(tl.bfloat16)
            G_low = G_acc.to(tl.bfloat16)
        elif LOWP_KIND == 0:
            H_low = H_blk.to(tl.float16)
            G_low = G_acc.to(tl.float16)
        else:
            H_low = H_blk
            G_low = G_acc
        accY += tl.dot(H_low, G_low)  # [BL,BH]
...
# ============================================================
@triton.jit
def fused_ffn_phase13_geglu_preg(
    P_ptr, V1_ptr, G_ptr, Y_ptr, b1_ptr, b2_ptr,
    B, L, D, R1, Hdim,
    sP_b, sP_l, sP_r1,
    sV1_r1, sV1_d,
    sG_d, sG_h,
    sY_b, sY_l, sY_h,
    sb1, sb2,
    BL: tl.constexpr, BD: tl.constexpr, BR1: tl.constexpr, BH: tl.constexpr,
    LOWP_KIND: tl.constexpr,  # 0=fp16, 1=bf16, 2=fp32/no-cast
    USE_TANH: tl.constexpr,
    num_warps: tl.constexpr,
    num_stages: tl.constexpr,
    ...
    for d0 in range(0, D, BD):
        ...
        if LOWP_KIND == 1:
            ...
            H_low = H_blk
        accY += tl.dot(H_low, G_blk)

    b2_blk = tl.load(b2_ptr + offs_h * sb2, mask=m_h, other=0.0).to(tl.float32)
    mask = (offs_l[:, None] < L) & m_h[None, :]
    tl.store(
...
def flashsvd_ffn_geglu_two_stage(
    ...
    LOWP_KIND = _lowp_kind_from_dtype(P.dtype)
...
def flashsvd_ffn_geglu_fused_preg(
    ...
    Hdim = G.shape[1]
    if G.shape[0] != D:
        raise ValueError(f"G shape mismatch: expected [D, H] with D={D}, got {tuple(G.shape)}")
    ...
    strides = dict(
        sP_b=P.stride(0), sP_l=P.stride(1), sP_r1=P.stride(2),
        sV1_r1=V1.stride(0), sV1_d=V1.stride(1),
        sG_d=G.stride(0), sG_h=G.stride(1),
        sY_b=Y.stride(0), sY_l=Y.stride(1), sY_h=Y.stride(2),
        sb1=b1.stride(0), sb2=b2.stride(0),
    )
    grid = (B, triton.cdiv(L, BL), triton.cdiv(Hdim, BH))
    ...
    fused_ffn_phase13_geglu_preg[grid](
        P, V1, G, Y, b1, b2,
        B, L, D, R1, Hdim,
        *strides.values(),
        BL, BD, BR1, BH,
...
def main():
    ...
    _ = flashsvd_ffn_geglu_fused_preg(P, V1, G, b1, b2, BL, BD, BR1, BH)
    ...
    Y_preg  = flashsvd_ffn_geglu_fused_preg(P, V1, G, b1, b2, BL, BD, BR1, BH)

    for name, y in [("two-stage", Y_two), ("fused", Y_fused), ("fused-preG", Y_preg)]:
    ...
    preg_t = bench(lambda: flashsvd_ffn_geglu_fused_preg(P, V1, G, b1, b2, BL, BD, BR1, BH))
    ...
    print("FlashSVD (Triton fused preG): "
          f"mean {preg_t['mean_ms']:.2f} ms | median {preg_t['median_ms']:.2f} ms | p95 {preg_t['p95_ms']:.2f} ms "
          f"| {tps(preg_t['mean_ms']):,.0f} tok/s")
