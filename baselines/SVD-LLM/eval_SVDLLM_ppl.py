...
def main() -> None:
    ...
    def _run_one(model, tokenizer, label: str, datasets_override: Optional[List[str]] = None):
        perf["runs"].setdefault(label, {"phases": {}})

        _perf_to = PerfRecorder(args.device, label=f"{label}:to_device")
        _perf_to.__enter__()
        model = _to_device(model, args.device, args.dtype)
        model.eval()
        _perf_to.__exit__(None, None, None)
        perf["runs"][label]["phases"]["to_device"] = _perf_to.to_dict()
        ...
        if "token" in metrics:
            _perf_tok = PerfRecorder(args.device, label=f"{label}:token_ppl")
            _perf_tok.__enter__()
            ...
            _perf_tok.__exit__(None, None, None)
            perf["runs"][label]["phases"]["token_ppl"] = _perf_tok.to_dict()
        ...
        if any(m in metrics for m in ("word", "byte", "bpb")):
            ...
            _perf_lme = PerfRecorder(args.device, label=f"{label}:lm_eval")
            _perf_lme.__enter__()
            ...
            _perf_lme.__exit__(None, None, None)
            perf["runs"][label]["phases"]["lm_eval"] = _perf_lme.to_dict()
    ...
    if args.compare_dobi:
        ...
        perf["models"].setdefault("ours", {"phases": {}})
        _perf_load = PerfRecorder(args.device, label="ours:load_model")
        _perf_load.__enter__()
        model, tokenizer = _load_ours(args.checkpoint)
        _perf_load.__exit__(None, None, None)
        perf["models"]["ours"]["phases"]["load_model"] = _perf_load.to_dict()
        ...
        perf["models"].setdefault("dobi", {"phases": {}})
        _perf_load = PerfRecorder(args.device, label="dobi:load_model")
        _perf_load.__enter__()
        ...
        _perf_load.__exit__(None, None, None)
        perf["models"]["dobi"]["phases"]["load_model"] = _perf_load.to_dict()
        models_info.append({
            "name": "dobi",
