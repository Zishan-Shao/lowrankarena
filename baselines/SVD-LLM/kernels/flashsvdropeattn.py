#!/usr/bin/env python3
"""Low-level FlashSVD RoPE kernel and legacy harness.

Production component code now launches this kernel directly from
`flashsvd_component/svd_llama.py`. Keep this module focused on kernel
primitives, benchmarking, and compatibility.
"""
...
# Optional dependency used only in reference path; kernel does not require it
try:
...
class FlashSVDRoPEAttention(nn.Module):
    ...
    def forward(
        ...
        def _build_cos_sin_bmd(B, M, dh, position_ids, device, dtype):
            # HF LlamaModel passes position_ids as [1, M] even for B>1 and relies on broadcasting.
            # Support both [B,M] and [1,M] without materializing full B copies.
            if position_ids is None:
                position_ids = torch.arange(M, device=device, dtype=torch.long).unsqueeze(0)
            if position_ids.dim() == 1:
                position_ids = position_ids.unsqueeze(0)
            if position_ids.shape[-1] != M:
                raise ValueError(f"position_ids last dim {position_ids.shape[-1]} != M {M}")

            shared_batch = position_ids.shape[0] == 1 and B > 1
            if not shared_batch and position_ids.shape[0] != B:
                if position_ids.shape[0] == 1:
                    shared_batch = True
                else:
                    raise ValueError(f"position_ids batch dim {position_ids.shape[0]} != B {B}")

            # Preferred path: build from inv_freq (works for HF + our custom rotary emb)
            inv = getattr(self.rotary_emb, "inv_freq", None)
            if inv is not None:
                inv = inv.to(device=device, dtype=torch.float32)
                pos = position_ids.to(torch.float32)[..., None]  # [Bpos, M, 1]
                ang = pos * inv  # [Bpos, M, dh/2]
                cos_h = torch.cos(ang)
                sin_h = torch.sin(ang)
                cos = torch.cat((cos_h, cos_h), dim=-1).to(dtype)  # [Bpos, M, dh]
                sin = torch.cat((sin_h, sin_h), dim=-1).to(dtype)
            else:
                # Fallback: call rotary_emb forward if it supports (x, position_ids) (HF style),
                # or (x, seq_len=...) (our cached impl).
                bpos = int(position_ids.shape[0])
                dummy = torch.empty((bpos, M, dh), device=device, dtype=dtype)
                try:
                    cos, sin = self.rotary_emb(dummy, position_ids)  # HF: forward(x, position_ids)
                except TypeError:
                    # Cached impl: forward(x, seq_len)
                    cos, sin = self.rotary_emb(dummy, seq_len=M)
                    # Expected cached shape [1,1,M,dh] -> [1,M,dh]
                    if cos.dim() == 4:
                        cos = cos[:, 0, :, :]
                        sin = sin[:, 0, :, :]
        ...
        if attention_mask is not None:
            if attention_mask.dim() == 2:
                pad_mask = attention_mask   # [B, M]
                pad_mask_ptr = pad_mask
                has_pad = 1
            elif attention_mask.dim() == 4:
                add_mask = attention_mask   # [B, 1, M, M]
