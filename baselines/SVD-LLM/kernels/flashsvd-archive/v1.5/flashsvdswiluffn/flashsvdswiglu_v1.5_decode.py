...
def _load_flashsvdswiglu_v1():
    path = Path(__file__).resolve().parents[2] / "v1" / "flashsvdswiluffn" / "flashsvdswiglu_v1.py"
    spec = importlib.util.spec_from_file_location("flashsvdswiglu_v1_archive", str(path))
    if spec is None or spec.loader is None:
        raise ImportError(f"Failed to load flashsvdswiglu_v1 from {path}")
...
def main():
    ...
    # ---- optional: compare against archive v1 implementation ----
    try:
        sw1 = _load_flashsvdswiglu_v1()
