...
def flashsvd_rope_decode_splitk_stage1_packed(
    ...
    for nk_off in range(0, SPLIT_K, BN):
        ...
        # NOTE: tl.dot requires M/N/K >= 16 on recent Triton for tensor-core path.
        # Here M=REP can be < 16 (GQA), so use explicit mul+reduce.
        ...
        m_curr = tl.max(scores, axis=1)
        m_new = tl.maximum(m_i, m_curr)
        # Guard against the all-masked case where m_new=-inf (would produce NaNs).
    ...
    # Triton may reject scalar indexing with constexpr in some versions, so store via vector pointers.
    tl.store(
...
def flashsvd_attn_decode_packed(
    ...
    *,
    ...
    # workspace: float32 states (optionally preallocated for decode loops)
    ...
    # output: [B, H, Dh] (optionally preallocated for decode loops)
    if out is None:
