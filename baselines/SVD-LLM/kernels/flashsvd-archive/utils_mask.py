...
def load_tiles(
    ...
    row_ok = (row_offset + offs_x) < full_len
    ...
    for r_start in range(0, r_dim, BLOCK_R):
        ...
        P_sub = tl.load(P_ptrs, mask=row_ok[:, None] & mask_r[None, :], other=0.).to(tl.float32)
...
def _demo_attn_kernel(
    ...
    for start_n in range(0, seqlen, BLOCK_M):
        ...
        mask_i32  = tl.load(mask_ptrs, mask=(start_n + offs_n) < seqlen, other=0).to(tl.int32)
        ...
        acc      = acc * exp_diff[:, None] + tl.dot(p, v)
        m_i      = m_new
