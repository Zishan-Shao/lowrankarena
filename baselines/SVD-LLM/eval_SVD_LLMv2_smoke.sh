...
mkdir -p outputs/svdllmv2/smoke

SMOKE_MODEL=baselines/SVD-LLM/checkpoints_smoke/jeffwan_llama_7b_hf_svdllmv2_keep0.4_smoke.pt
...
CUDA_VISIBLE_DEVICES="${GPU}" python eval_results/eval_benchmarks.py \
  ...
  --force_right_padding \
  --fix_pad_query_mask \
  --output_json outputs/svdllmv2/smoke/jeffwan_llama-7b-hf_svdllmv240_smoke_lm_eval.json
...
CUDA_VISIBLE_DEVICES="${GPU}" python eval_results/eval_general_ppl.py \
  ...
  --device "${DEVICE}" \
  --dtype "${DTYPE}" \
  --output_json outputs/svdllmv2/smoke/jeffwan_llama-7b-hf_svdllmv240_smoke_ppl.json
...
CUDA_VISIBLE_DEVICES="${GPU}" python eval_results/eval_linguistic_tasks.py \
  ...
  --device "${DEVICE}" \
  --dtype "${LING_DTYPE}" \
  --output_json outputs/svdllmv2/smoke/jeffwan_llama-7b-hf_svdllmv240_smoke_ling.json
