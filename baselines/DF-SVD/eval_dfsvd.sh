CUDA_VISIBLE_DEVICES=6 python -m eval_results.eval_benchmarks \
  --model ./baselines/DF-SVD/huggyllama_llama7b_dfsvd_0.4 \
  --dfsvd_base_model huggyllama/llama-7b \
  --device cuda --batch_size 1 --dtype bfloat16 \
...
CUDA_VISIBLE_DEVICES=6 python -m eval_results.eval_general_ppl \
  --dfsvd_model ./baselines/DF-SVD/huggyllama_llama7b_dfsvd_0.4 \
  --dfsvd_base_model huggyllama/llama-7b \
  --datasets wikitext2,ptb,c4 \
  --device cuda --seqlen 2048 --batch_size 1 --dtype bfloat16 \
    --metrics token \
