...
def main() -> None:
    ...
    for name, model_path in model_items:
        ...
        perf["models"][name] = {"phases": {}, "datasets": {}}
        t0 = time.time()
        _perf_load = PerfRecorder(args.device, label=f"{name}:load_model")
        ...
        perf["models"][name]["phases"]["load_model"] = _perf_load.to_dict()
        ...
        if want_token:
            for ds_name in datasets:
                _perf_ds = PerfRecorder(args.device, label=f"{name}:token_ppl:{ds_name}")
                _perf_ds.__enter__()
                packed = pack_dataset(
                    ds_name,
                    tokenizer,
                ...
                _perf_ds.__exit__(None, None, None)
                perf["models"][name]["datasets"][ds_name] = _perf_ds.to_dict()
        ...
        if want_lm_eval:
            ...
            if lm_eval_datasets:
                _perf_lme = PerfRecorder(args.device, label=f"{name}:lm_eval")
                _perf_lme.__enter__()
                ...
                perf["models"][name]["phases"]["lm_eval"] = _perf_lme.to_dict()
                if res is None:
