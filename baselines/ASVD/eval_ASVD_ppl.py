...
def main() -> None:
    ...
    p.add_argument("--hf_token", type=str, default=None, help="HuggingFace token (if needed)")
    ...
    for name, model_path in model_items:
        ...
        if want_lm_eval:
            try:
                ...
                raise RuntimeError(
                    "lm-eval-harness is required for word/byte/bpb. Install with: pip install lm-eval\n"
                    f"Original error: {e}"
                )
