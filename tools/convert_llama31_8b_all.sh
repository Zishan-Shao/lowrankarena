#!/usr/bin/env bash
# Batch convert all Llama-3.1-8B (base) .pt checkpoints to HF directories.
# Run from: lowrankarena/
#
# Usage:
#   bash tools/convert_llama31_8b_all.sh [--dtype bf16]
...
SVDLLM_PT="baselines/SVD-LLM/checkpoints/svdllm/llama31_8b"
ASVD_PT="baselines/ASVD/checkpoints/asvd/llama31_8b"
HF_BASE="hf_ckpts/LowRankArena/llama31_8b"
...
for pt in "$SVDLLM_PT"/meta_llama_Llama_3.1_8B_*.pt; do
    [[ -f "$pt" ]] || continue
    base=$(basename "$pt" .pt | sed 's/meta_llama_Llama_3\.1_8B_//')
...
# ── ASVD ─────────────────────────────────────────────────────────────────────
for pt in "$ASVD_PT"/Llama_3.1_8B_asvd_raw_*.pt; do
    [[ -f "$pt" ]] || continue
