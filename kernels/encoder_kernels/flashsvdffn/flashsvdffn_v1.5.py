...
def fused_ffn_full_batched_bias(
    ...
    # Dtype for matmuls (use tensor cores for fp16/bf16)
    in_dtype = tl.bfloat16 if USE_BF16 else tl.float16

    # Rank-space accumulator: S = sum_d gelu(...) @ U2  -> [BL, R2_PAD]
    # Note: Triton doesn't support Python list comprehensions inside @triton.jit.
    offs_r2 = tl.arange(0, R2_PAD)
    m_r2 = offs_r2 < R2
    acc_r = tl.zeros((BL, R2_PAD), dtype=tl.float32)

    # pre-activation accumulator (per D-block)
    ...
    for d0 in range(0, D, BD):
        ...
        # 2) Accumulate rank-space S = T @ U2  (full R2_PAD tile)
        U2_sub = tl.load(  # [BD, R2_PAD]
            U2_ptr + d[:, None] * sU2_d + offs_r2[None, :] * sU2_r2,
            mask=m_d[:, None] & m_r2[None, :],
            other=0.0,
        ).to(in_dtype)
        acc_r = tl.dot(T, U2_sub, acc=acc_r, out_dtype=tl.float32)  # [BL, R2_PAD]

    # 3) Single lift: out = S @ V2 + b2
    acc_r_f = acc_r.to(in_dtype)
    m_l = offs_l < L
    base = C_ptr + offs_Cb + offs_l[:, None] * sC_l
    for h0 in range(0, H, BH):
        offs_h = h0 + tl.arange(0, BH)
        V2_sub = tl.load(  # [R2_PAD, BH]
...
def flashsvd_ffn(
    ...
    *,
    ...
    if P.dtype not in (torch.float16, torch.bfloat16):
        raise TypeError(f"Unsupported dtype {P.dtype}; expected fp16/bf16.")
    # tl.dot constraints (tensor cores path): M,N,K >= 16
    for name, val in (("BL", BL), ("BD", BD), ("BH", BH), ("BR1", BR1), ("BR2", BR2)):
        if int(val) < 16:
    ...
    # We compute the rank-space intermediate once per (B, L-block) and then
    # loop over H blocks inside the program to avoid recomputing it for every pid_h.
...
def main():
    ...
    for _ in range(num_runs):
        ...
        torch.cuda.reset_peak_memory_stats(device)
        start_evt.record()
        Y1 = F.gelu(P.matmul(V1) + b1.unsqueeze(0).unsqueeze(0))
        Y2 = Y1.matmul(U2)
