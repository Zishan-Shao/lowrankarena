...
def build(request: CompressionRequest) -> CompressionArtifact:
    ...
    return save_artifact(
        ...
        baseline=baseline,
        command=command,
        output_format="mask_then_hf_export",
        status="planned",
        ready_for_load=False,
        notes=(
            "Bonsai planning adapter. Native Bonsai emits masks, so unified "
            "execution remains gated until the HF materialization step is integrated."
        ),
    )
