...
def build(request: CompressionRequest) -> CompressionArtifact:
    ...
    return save_artifact(
        ...
        baseline=baseline,
        command=command,
        output_format="sparse_then_hf_export",
        status="planned",
        ready_for_load=False,
        notes=(
            "Wanda-SP planning adapter. Unified execution is gated until sparse "
            "weights and runtime metadata are exported through the artifact contract."
        ),
    )
