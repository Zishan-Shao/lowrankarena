...
class SVDTransformLayer(nn.Module):
    def __init__(self,gamma=None, weight = None, bias = None, name = None, device = None):
        ...
        rank = max(1, min(int(gamma), W_T.size(0), W_T.size(1)))
        U, S, V = torch.svd_lowrank(W_T, q=rank, niter = 10)
        diag_S = torch.diag(S)
        sqrt_S = torch.sqrt(diag_S)
        A_weight_T = (U @ sqrt_S).to(torch.float16)
        B_weight_T = (sqrt_S @V.T).to(torch.float16)
        if bias is None: 
            self.ALinear = nn.Linear(W_T.size(0), rank, bias=False, device = device)
            self.BLinear = nn.Linear(rank, W_T.size(1), bias=False, device = device)
        else:
            self.ALinear = nn.Linear(W_T.size(0), rank, bias=False, device = device)
            self.BLinear = nn.Linear(rank, W_T.size(1), bias=True, device = device)
