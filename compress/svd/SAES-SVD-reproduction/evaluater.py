...
def ppl_eval(
    ...
    # Some quantized model wrappers (e.g., AutoAWQ) may not support `.to()` after packing.
    try:
        model.to(device)
    except Exception as e:
        print(f"[Warn] model.to({device}) failed: {e}. Assuming model is already on the correct device.")
    try:
        model.eval()
    except Exception:
        pass

    # Record weight (model parameters) memory after move
    weight_mem_mib = 0.0
    peak_alloc_mib = None
    peak_reserved_mib = None
    if torch.cuda.is_available() and str(device).startswith("cuda"):
        weight_mem_mib = torch.cuda.memory_allocated() / (1024**2)
    ...
    for dataset in datasets:
        ...
        for i, batch in enumerate(tqdm(test_loader)):
            ...
            # Avoid relying on possibly incompatible config flags on old checkpoints
            output = model(
                batch,
                use_cache=False,
            ...
            if torch.isfinite(lm_logits).all():
                ...
                batch_tokens = int(shift_labels.numel())
                loss_sum += float(loss.item())
                token_count += int(batch_tokens)
                if int(debug_batches) > 0 and i < int(debug_batches):
                    head = batch_cpu[0, : max(1, int(debug_tokens))].tolist()
                    batch_mean_ce = float(loss.item()) / float(max(1, batch_tokens))
                    print(
                        f"[PPLDebug] dataset={dataset} batch={i} shape={tuple(batch_cpu.shape)} "
                        f"tokens_head={head} batch_mean_ce={batch_mean_ce:.6f}"
                    )
        if token_count <= 0:
            ppls[dataset] = float("nan")
            continue
        mean_nll = float(loss_sum / float(token_count))
        ppl = np.exp(mean_nll)
        ppls[dataset] = ppl
        if int(debug_batches) > 0:
            print(
                f"[PPLDebug] dataset={dataset} total_tokens={token_count} "
                f"mean_nll={mean_nll:.6f} ppl={ppl:.6f}"
            )

    # Capture peak memory during evaluation
    if torch.cuda.is_available() and str(device).startswith("cuda"):
    ...
    if torch.cuda.is_available() and str(device).startswith("cuda"):
        ...
        print("Peak Memory (allocated): {:.2f} MiB".format(peak_alloc_mib))
        print("Peak Memory (reserved):  {:.2f} MiB\n".format(peak_reserved_mib))
