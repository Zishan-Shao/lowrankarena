...
def factorize_layer_kron_svd(
    ...
    alpha_increase_factor: float = 1e-1,
...
def compress_llama_with_kron_svd(
    ...
    part_of = ["model.layers.24.mlp.gate_proj", "model.layers.24.mlp.down_proj", "model.layers.25.mlp.up_proj", "model.layers.25.mlp.down_proj", "model.layers.26.mlp.gate_proj", "model.layers.26.mlp.down_proj", "model.layers.27.mlp.gate_proj", "model.layers.27.mlp.down_proj", "model.layers.28.mlp.up_proj", "model.layers.28.mlp.down_proj"]
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear) and (name in part_of):
            name = name +".weight"
            print ("layer", name)
    ...
    for layer_name, layer_module in tqdm(linear_layers_to_compress.items(), desc="Compressing Layers"):
        ...
        param_ratio = 1.0#llsensitivity_dict[layer_name]
...
if __name__ == "__main__":
    ...
    KRON_FACTORS_DIR = "/home/jovyan/shares/SR004.nfs2/chekalina/FisherKronecker/grads_output/Llama-3.1-8B-Instruct/fisher_factors_output_1404"
    OUTPUT_DIR = f"./{MODEL_NAME.split('/')[-1]}-kron-svd-sensitive"
    SENSITIVITY_DICT_PATH = "/home/jovyan/shares/SR004.nfs2/kurkin/FisherKronecker/layers_min_ratio_unsloth_Llama-3.1-8B-Instruct.json"
    RANK_ALIGNMENT = 8
    MIN_RANK = 1
    ...
    #logging.info(f"Model compression statistics:")
    #logging.info(f"  Processed layers: {compression_stats['layers_processed']}")
    #logging.info(f"  Skipped layers: {compression_stats['layers_skipped']}")
    #logging.info(f"  Failed layers: {compression_stats['layers_failed']}")
    #logging.info(f"  Compression ratio: {compression_stats['model_stats']['compression_ratio']:.4f}")
