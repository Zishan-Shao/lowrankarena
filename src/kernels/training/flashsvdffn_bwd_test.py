...
# Tunables / defaults
# -----------------------------------
BLOCK_M = 64     # rows (sequence/tokens)
BLOCK_N = 64     # columns (D_h or D_out tiles)
BLOCK_K = 64     # input features tile (D_in)
...
# -----------------------------------
_BWD2_CONFIGS = [
    triton.Config({'BLOCK_M': 64, 'BLOCK_N': 64, 'BLOCK_K': 64, 'BLOCK_R': 32}, num_warps=4, num_stages=2),
...
def check_correctness_and_bench(
    ...
    _ = torch.randn(1, device=device)  # init CUDA ctx

    # Init factors with variance ~ 1
    scale1 = 1.0 / math.sqrt(R1)
    scale2 = 1.0 / math.sqrt(R2)
    X  = torch.randn(B, M, Din, device=device, dtype=dtype)

    P1 = torch.randn(Din, R1, device=device, dtype=dtype)
    V1 = torch.randn(R1, Dh,  device=device, dtype=dtype) * scale1
    b1 = torch.zeros(Dh,      device=device, dtype=dtype)

    P2 = torch.randn(Dh,  R2,   device=device, dtype=dtype)
    V2 = torch.randn(R2,  Dout, device=device, dtype=dtype) * scale2
