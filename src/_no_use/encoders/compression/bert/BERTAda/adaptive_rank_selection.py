...
class MaskedSVDLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Ensure dtype matches input for fp16 compatibility
        ms  = (m * self.s).to(x.dtype)                       # [R]
        US  = (self.U * ms.unsqueeze(0)).to(x.dtype)         # [in,R]
        mid = torch.matmul(x, US)                            # [...,R]
        y   = torch.matmul(mid, self.V.to(x.dtype).t())      # [...,out]
...
def train_ars(
    ...
    with torch.no_grad():
        ...
        # Finalize masks: convert to binary top-k masks and set permanently
        final_masks = []
        for m_soft, k in zip(masks_soft, ks):
            # Create binary mask with top-k entries set to 1
            m_binary = torch.zeros_like(m_soft)
            if k > 0:
                m_binary[:k] = 1.0
            final_masks.append(m_binary)

        # Set final masks permanently on each module
        for mod, m_final in zip(masked_modules, final_masks):
            mod._current_mask = m_final
