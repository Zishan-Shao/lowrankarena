...
def main(args):
    ...
    # Load Pruned Model (handle PyTorch>=2.6 weights_only default and safe globals)
    try:
        ...
        # Allowlist common HF classes then retry
        try:
            ...
            allow = [LlamaForCausalLM]
            if MistralForCausalLM: allow.append(MistralForCausalLM)
            if OPTForCausalLM: allow.append(OPTForCausalLM)
            if PreTrainedTokenizerFast: allow.append(PreTrainedTokenizerFast)
            if PreTrainedTokenizer: allow.append(PreTrainedTokenizer)
            add_safe_globals(allow)
        except Exception:
            pass
        pruned_dict = torch.load(args.prune_model, map_location='cpu', weights_only=False)
    tokenizer, model = pruned_dict['tokenizer'], pruned_dict['model']
    # Compatibility with newer Transformers configs and models
    try:
        _ = getattr(model.config, "_attn_implementation")  # probe property
    except Exception:
        pass
    if not hasattr(getattr(model, "config", object()), "_attn_implementation_internal"):
        try:
            model.config._attn_implementation_internal = "eager"
        except Exception:
            pass
    # Ensure rotary_emb exists on LlamaModel if missing
    try:
        from transformers.models.llama.modeling_llama import LlamaRotaryEmbedding
        base = getattr(model, 'model', None)
        if base is not None and not hasattr(base, 'rotary_emb'):
            base.rotary_emb = LlamaRotaryEmbedding(config=model.config)
            try:
                emb_w = base.embed_tokens.weight
                base.rotary_emb.to(device=emb_w.device, dtype=emb_w.dtype)
            except Exception:
                pass
    except Exception:
        pass
