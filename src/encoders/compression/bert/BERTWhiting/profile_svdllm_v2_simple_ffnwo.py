...
task_name = "sst2"
MODEL_DIR = "textattack/bert-base-uncased-sst-2"  # Use HuggingFace model directly
...
# -----------------------------------------------------------------------------
# 4.5) SVD-LLM v2: local update (FFN only)
# -----------------------------------------------------------------------------
@torch.no_grad()
def svdllm_v2_local_update_ffn_only(
    student,
    teacher,
    loader,
    device: str,
    ...
    """
    SVD-LLM v2 local update (FFN only, stable version).

    IMPORTANT for this codebase's parameterization:
      FFN linears are implemented as:  Y ≈ (X @ U) @ V + b
    So v2 local update is done by:
      - Fix U (input-side projection, keeps whitening geometry)
      - Update V (output-side projection) via ridge least squares using teacher IO pairs.

    To keep peak memory low, we do NOT store full (X, Y). Instead, we accumulate
      A = sum(Z^T Z),  B = sum(Z^T (Y - b)), where Z = X @ U
    Then solve (A + ridge I) V = B.
    """
    student.eval()
    teacher.eval()
    ...
    num_layers = len(student.bert.encoder.layer)

    for i in range(num_layers):
        shim = student.bert.encoder.layer[i]
        blk = shim.block               # LayerShim stores SVDBlock at .block
        t_layer = teacher.bert.encoder.layer[i]

        # Fix U (student)
        U1 = blk.U1.detach().to(device=device, dtype=torch.float32)   # [dm, r1]
        U2 = blk.U2.detach().to(device=device, dtype=torch.float32)   # [dff, r2]

        # Bias (student); match broadcasting against [N, d_out]
        b1 = blk.b1.detach().to(device=device, dtype=torch.float32).view(1, -1)  # [1, dff]
        b2 = blk.b2.detach().to(device=device, dtype=torch.float32).view(1, -1)  # [1, dm]
        ...
        # Cleanup to keep peak memory low
        del A1, B1, A2, B2, V1_new, V2_new, U1, U2, b1, b2
        torch.cuda.empty_cache()

        print(f"[v2] updated FFN V (online) at layer {i}")
...
def svdllm_v2_simple_local_update_ffn_wo_V(
    ...
    for i in range(num_layers):
        ...
        # cleanup
        del A1,B1,A2,B2,Ao,Bo,V1_new,V2_new,Vo_new,U1,U2,Uo,b1_t,b2_t,bo_t
        torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    teacher = BertForSequenceClassification.from_pretrained(MODEL_DIR, config=cfg).to(device).eval()
    ...
    print("Calibrating input covariances (Whiting)…")
    ...
    print(f"Persistent low-rank model storage (Whiting): {baseline_bytes/1024**2:6.1f} MiB")
