# profile_svdllm_v1.py — SVD-LLM v1 for RoBERTa (Whitening-SVD)
# -------------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    print(f"Persistent low-rank model storage (RoBERTa Whitening): {baseline_bytes/1024**2:6.1f} MiB")

    # ─── Evaluate ────────────────────────────────────────────────────────────
    metric_name = "pearson" if task_name == "stsb" else "acc"
    acc, peak_lr, t = acc_peak_time(model, loader, device, task_name)
    print(f"RoBERTa Whitening v1 | {metric_name}={acc:.4f} | peak ={peak_lr:6.1f} MiB | {t:6.1f} ms/b")
