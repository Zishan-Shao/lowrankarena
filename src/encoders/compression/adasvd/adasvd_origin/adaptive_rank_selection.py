...
def collect_op_metadata(linear_list: List[Tuple[str, nn.Linear]]) -> List[List[float]]:
    """
    Build fixed metadata features per op: [in_norm, out_norm, one_hot_type×11].
    Shape: [L, 13].

    Type keys (index 0-9, 10=other):
      BERT / RoBERTa:
        ...
        5: output.dense
      ModernBERT:
        6: attn.Wqkv   (fused Q/K/V)
        7: attn.Wo     (attention output projection)
        8: mlp.Wi      (FFN input / GeGLU gate+linear)
        9: mlp.Wo      (FFN output projection)
      10: other
    """
    type_keys = [
        "attention.self.query",    # 0 (BERT Q)
        "attention.self.key",      # 1 (BERT K)
        "attention.self.value",    # 2 (BERT V)
        "attention.output.dense",  # 3 (BERT Wo_attn)
        "intermediate.dense",      # 4 (BERT Wi)
        "output.dense",            # 5 (BERT Wo_ffn)
        "attn.Wqkv",               # 6 (ModernBERT fused QKV)
        "attn.Wo",                 # 7 (ModernBERT Wo_attn)
        "mlp.Wi",                  # 8 (ModernBERT Wi / GeGLU)
        "mlp.Wo",                  # 9 (ModernBERT Wo_ffn)
    ]                              # 10 = other
    max_dim = 4096.0
    meta = []
    ...
    return meta  # List[List[float]], shape [L, 13]
