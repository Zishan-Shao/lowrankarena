...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="experiments/glue")
    parser.add_argument("--out_csv", default="experiments/encoder_runs.csv",
...
def _write_csv_row(row_data: dict):
    """Append a row to expA_fine_tune.csv (accuracy-only; no mem/sps)."""
...
def _write_pretrain_csv_row(pretrain_dir, task, metric_name, metric_value, args):
    """Write pretrain baseline score to expA_fine_tune.csv."""
    _write_csv_row({
...
def _write_finetune_csv_row(checkpoint_path, task, results: dict, args):
    ...
    best_value_flash15 = results.get("best_value_flashsvd15")
    ...
    # Row 3: flashsvd15 backend (only if successfully measured)
    if best_value_flash15 is not None:
        _write_csv_row({**base_row,
                        "backend": "flashsvd15",
                        "metric_name": metric_name,
                        "metric_value": f"{best_value_flash15:.6f}"})
        print(f"[finetune] CSV row written (flashsvd15): {metric_name}={best_value_flash15:.4f}")
...
def _write_compress_eval_csv_rows(task, comp_info, metric_name,
                                   ...
                                   sdpa_path=None):
    """Write stage1 (compress → eval, no finetune) accuracy rows to expA_fine_tune.csv."""
    ts = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
    base = {
        "timestamp": ts,
...
def _append_flashsvd_csv_row(task, comp_info, speed_metrics, metric_name, metric_value,
                              csv_path="experiments/encoder_runs.csv",
                              backend="flashsvd"):
...
def evaluate_compressed_model(checkpoint_path: Path, task: str, args) -> Dict:
    ...
    # Write stage1 accuracy rows to expA_fine_tune.csv
...
def finetune_on_task(checkpoint_path: Path, task: str, args) -> Dict:
    ...
    from src.encoders.backend import FlashSVDBlock as _FSB, FlashSVD15Block as _FSB15
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in pbar:
            ...
            with torch.autocast("cuda", dtype=_dtype, enabled=_use_autocast):
                outputs = model(**batch)
                loss = outputs.loss
    ...
    if _comp_method != 'dense' and getattr(args, 'qkv_mode', 'per_head') == 'per_head':
        ...
        try:
            ...
            model.eval()
            torch.set_grad_enabled(False)
            print(f"\n[eval] Final evaluation (flashsvd15)...")
            final_results_flashsvd15, _ = evaluate_task(model, val_loader, task, device, original_model_id=original_model_id)
            print(f"[eval] After fine-tuning (flashsvd15): {final_results_flashsvd15}")
        except RuntimeError as e:
            print(f"[eval] FlashSVD15 unavailable for final eval: {e}")
    ...
    final_metric_flashsvd15 = (final_results_flashsvd15.get(cfg["metric"], 0)
                                if final_results_flashsvd15 is not None else None)
    ...
    return {
        ...
        "metrics": {
            ...
            "final_flashsvd15": final_results_flashsvd15,
            "best_value": best_metric_value,
            "best_value_flashsvd": final_metric_flashsvd,
            "best_value_flashsvd15": final_metric_flashsvd15,
        ...
        "final_results_flashsvd15": final_results_flashsvd15,
        "best_metric": cfg["metric"],
        "best_value": best_metric_value,
        "best_value_flashsvd": final_metric_flashsvd,
        "best_value_flashsvd15": final_metric_flashsvd15,
...
def run_pipeline(args):
    ...
    for task in args.tasks:
        try:
            ...
            # Clean up GPU memory to prevent accumulation causing CUDA errors
            import torch
            import gc
            if torch.cuda.is_available():
                # Clear cache
                ...
                torch.cuda.synchronize()
                # Reset memory statistics (optional, helps monitoring)
                torch.cuda.reset_peak_memory_stats()
