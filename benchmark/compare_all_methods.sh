...
#   TWO_STAGE=true BACKENDS="flashsvd naive" bash benchmark/compare_all_methods.sh
#
#   # Test specific tasks only
#   TASKS="rte mrpc cola" bash benchmark/compare_all_methods.sh
#
...
AUTO_FIGURES="${AUTO_FIGURES:-false}"  # Automatically collect results and regenerate figures after all methods finish
...
NUM_RUNS="${NUM_RUNS:-1}"
PERF_CSV="${PERF_CSV:-experiments/encoder_runs.csv}"
...
# Stages
# When TWO_STAGE=true: runs both stages (no_finetune to see post-compression accuracy, with_finetune to see post-tuning accuracy)
# Normal mode: only runs no_finetune by default (quick verification)
# pretrain_before_compress mode: only runs with_finetune by default (fine-tuning is necessary)
#   But if TWO_STAGE=true is also set, also runs no_finetune (to observe compression loss)
...
run_one() {
  ...
  if [[ "${stage}" == "no_finetune" ]]; then
    ...
    reuse_checkpoint="true"   # Stage 2: reuse the stage-1 checkpoint to avoid recompression
  fi
  ...
  _ENV_CHECKED=true   # After the first check completes, all subsequent ones are skipped
...
SUCCESS=0
FAIL=0
_ENV_CHECKED=false   # Run the check on the first call to one_click_glue.sh; skip afterward
...
if [ "${AUTO_FIGURES}" = "true" ]; then
  ...
  python "benchmark/analysis/collect_glue_results.py" 2>&1 | tee -a "${SUMMARY_LOG}" && \
