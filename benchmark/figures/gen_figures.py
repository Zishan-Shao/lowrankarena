...
def _load_bf16_backend():
    ...
    with open(p, newline='', encoding='utf-8') as f:
        for r in _csv2.DictReader(f):
            ...
            try:
                mem.setdefault(b, []).append(float(r['peak_mem_mb']))
                thr.setdefault(b, []).append(float(r['throughput_sps']))
            except (KeyError, ValueError):
                pass
    out = {}
    for b in mem:
        out[b] = dict(mem=round(sum(mem[b])/len(mem[b]), 1),
                      thr=round(sum(thr[b])/len(thr[b]), 1))
    return out

_BF16 = _load_bf16_backend()
...
# Dense bf16: estimated from fp32 Dense × (bf16_naive / fp32_naive) ratio
_FP32_DENSE_MEM = 987
_FP32_NAIVE_MEM = 2004
_BF16_DENSE_MEM = round(_FP32_DENSE_MEM * _MEM('naive', _FP32_NAIVE_MEM) / _FP32_NAIVE_MEM)
...
# Dense bf16 throughput: not directly measured; using fp32 value scaled by bf16/fp32 naive ratio
_bf16_thr_naive = _THR('naive', 494.1)
_fp32_thr_naive = 193.8   # fp32 naive from archived measurements
_dense_thr_bf16 = round(265.7 * _bf16_thr_naive / _fp32_thr_naive, 1)
labels = ['Dense\n(HF BERT)*', 'Naive\n(einsum)', 'Naive\n(SDPA)', 'FlashSVD\n(v1)', 'FlashSVD\n(v1.5)']
values = [_dense_thr_bf16, _THR('naive',494), _THR('sdpa',1211), _THR('flashsvd',727), _THR('flashsvd15',1433)]
colors = [C['dense'], C['einsum'], C['sdpa'], C['flash'], C['flash15']]
...
_tv = values
_sdpa_vs_naive = f'+{(_tv[2]-_tv[1])/_tv[1]*100:.0f}%\nvs einsum'
_flash_vs_sdpa = f'{(_tv[3]-_tv[2])/_tv[2]*100:.0f}% vs SDPA'
ax.annotate(_sdpa_vs_naive, xy=(2, _tv[2]*0.92), xytext=(1.5, _tv[2]*0.55),
            ha='center', fontsize=10, fontweight='bold', color='#1a1a1a',
            arrowprops=dict(arrowstyle='->', color='#555', lw=1.2),
            bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                      edgecolor='#888888', alpha=0.95))
ax.annotate(_flash_vs_sdpa, xy=(2.5, _tv[3]), ha='center', fontsize=10,
            fontweight='bold', color=C['flash'])

ax.text(0.98, 0.97,
        '* Dense bf16 throughput estimated (not directly measured)\n'
        'FlashSVD v1.5: native bf16, highest throughput tier',
...
ax.set_title('Throughput — Attention Kernel Tiers\n'
             '(per-head ra48, seq=512, bs=32, bf16, avg across SVD/FWSVD methods)')
ax.set_ylim(0, max(values) * 1.35)
...
ax.text(1.9, (_flash_tot+_dense_tot)//2,
