...
'''
CUDA_VISIBLE_DEVICES=1 python expressivity/svd_act_lora_fast.py \
  --model meta-llama/llama-2-7b-hf \
  --keep_ratio 0.4 \
  --svd_method randomized --svd_niter 2 --svd_oversample 5 \
  --whitened_cache ./checkpoints/llama-2-7b-hf_whitening_only_fast_0.4.pt \
  --whitening_lm_datasets wikitext2,ptb,c4 \
  --trust_whitened_cache \
  --lora_nsamples 4096 --seqlen 2048 --train_batch_size 8 \
  --epochs 2 --lr 2e-4 --lora_rank 16 --lora_alpha 32 \
  --mix_buckets \
  --bucket_props LM:0.2,INST:0.6,MATH:0.2 \
  --bucket_lm_datasets c4 \
  --bucket_inst_datasets yahma/alpaca-cleaned,hellaswag,piqa,winogrande_xl,ai2_arc_easy,ai2_arc_challenge,openbookqa \
  --bucket_math_datasets mathqa,gsm8k \
  --save_path ./checkpoints/llama-2-7b-hf_act_lora_lmwhiten_mixedlora_fast_0.4.pt
...
def run_activation_lora(
    ...
    """
    Pipeline:
      1) Cholesky whitening -> SVD compression (keeps `keep_ratio` params).
    ...
    if whitened_cache is None:
        ...
        whitened_cache = os.path.join(
            "checkpoints", f"{model_stub}_whitening_only_{keep_ratio}.pt"
        )
        ...
        if not trust_whitened_cache:
            def _cache_shapes_match(m):
                try:
                    ...
                    return True
            if not _cache_shapes_match(model):
                print(f"[Cache] Cached model shapes mismatch current ratio; recomputing whitening instead of using {whitened_cache}.")
                model = None
                tokenizer = None
                cache_loaded = False
    if model is None:
    ...
    if cache_loaded:
        ...
        if whitened_cache is not None:
            os.makedirs(os.path.dirname(whitened_cache), exist_ok=True)
            torch.save({"model": model.cpu(), "tokenizer": tokenizer}, whitened_cache)
            model = model.to(dev)
    ...
    # When using SFT-style masked labels, prefer label-aware trainer (ignore full_seq_loss)
    effective_full_seq = (full_seq_loss and not sft_data_path)
    if epochs > 0:
...
def main():
    ...
    p.add_argument("--keep_ratio", type=float, default=0.8, help="Fraction of parameters kept after SVD (0-1)")
    ...
    p.add_argument(
        ...
        type=str,
        default="full",
        choices=["full", "truncated", "randomized"],
        help="SVD decomposition method: full (default), truncated (full SVD then top-k), randomized (svd_lowrank).",
    )
    p.add_argument("--svd_niter", type=int, default=2, help="Randomized SVD power iterations (svd_lowrank).")
    p.add_argument("--svd_oversample", type=int, default=5, help="Randomized SVD oversampling (svd_lowrank).")
    ...
    p.add_argument("--bucket_inst_datasets", type=str, default="yahma/alpaca-cleaned", help="Comma-separated instruction SFT datasets (Alpaca-style).")
